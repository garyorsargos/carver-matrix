...
- Navigate to the root project directory: `carver-matrix/`
- Setup your local development environment by running:
  ...
  - This command will automatically:
    ...
    1. Add the following entries to you `/etc/hosts` file:
       ...
       0.0.0.0   carver-app.zeus.socom.dev
       ```
    ...
    4. Watch the `ui/` directory for any code changes and automatically rebuild the `ui` container. Docker compose watch (which is the command being used) is a unique way we can harness the power of the hot-reloading development server that is provided by Vite. It watches for changes in your host machine `carver-app/ui/src` directory and syncs those changes to the `ui` container, which allows Vite to hot-reload your lastest additions.
...
- Login to the carver-app user interface
  - Navigate to: [https://carver-app.zeus.socom.dev](https://carver-app.zeus.socom.dev)
  - You will be redirected to a keycloak login page that says `"ZEUS-APPS"`
  - Login with:
    - Username: `user`
    - Password: `password`
  - You will then be redirected to the carver app user interface
...
- You should see all of the containers stop running:
  ...
  ✔ Network carver-app_carver-network  Removed     0.1s
  ```
...
| **User Interface (UI)**                 | Containerized version of the carver-app application's front-end running as a Single Page Application (SPA) served from an NGINX container (static file server) |
| **Application Program Interface (API)** | Containerized version of the carver-app application's back-end                                                                                                 |
